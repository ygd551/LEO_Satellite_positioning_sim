% 函数功能：对于有无VOR/DME辅助低轨单星定位结果对比 v
tic
clc;clear;
format long g

addpath(genpath("../../coordinateTransformation"));
paraAll = simulation_VORDMEcompare_para();
%%%%%%%%%%%%%%%%%%%参数设置%%%%%%%%%%%%%%%%%%%%%%
% 用户位置
...
VDOPVDByIterr        = [];
B0VDByIterr          = [];
errorVDXIteraMatrix  = zeros(paraAll.iterrT, 10);
errorVDYIteraMatrix  = zeros(paraAll.iterrT, 10);
errorVDZIteraMatrix  = zeros(paraAll.iterrT, 10);
...
%         errorPrList = [errorPrList, errorPr];
    ...
    0, 0, paraAll.withHeightFlag, paraAll.weightFlag,...
    ...
    paraAll.pseudoRangeError, paraAll.dopplerError, paraAll.DMEError, paraAll.VORError, paraAll.hError,...% 观测误差
    paraAll.withLEOFlag, paraAll.withLEOPrFlag, paraAll.withLEOPdFlag,...
    0, 1, paraAll.withHeightFlag, paraAll.weightFlag,...
    userECEFPositionStart, 0, 0, paraAll.clcOrder); 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [errorEast, errorNorth, errorHorizontal, errorVertical]=...
    errorECEF2ENU(calPos, userECEFPosition);
    errorVDByIterr       = [errorVDByIterr,      error];
    errorVDHoriByIterr   = [errorVDHoriByIterr,  errorHorizontal];
    errorVDEastByIterr   = [errorVDEastByIterr,  errorEast];
    errorVDEastByIterrJ  = [errorVDEastByIterrJ,  abs(errorEast)];
    errorVDNorthByIterr  = [errorVDNorthByIterr, errorNorth];
    errorVDNorthByIterrJ = [errorVDNorthByIterrJ, abs(errorNorth)];
    errorVDVetiByIterr   = [errorVDVetiByIterr,  errorVertical];
    errorVDVetiByIterrJ  = [errorVDVetiByIterrJ,  abs(errorVertical)];
    errorVDXByIterr      = [errorVDXByIterr,     calPos(1)-userECEFPosition(1)];
    errorVDYByIterr      = [errorVDYByIterr,     calPos(2)-userECEFPosition(2)];
    errorVDZByIterr      = [errorVDZByIterr,     calPos(3)-userECEFPosition(3)];
    B0VDByIterr          = [B0VDByIterr,         B00List(end)];
    PDOPVDByIterr        = [PDOPVDByIterr,       PDOP];
    HDOPVDByIterr        = [HDOPVDByIterr,       HDOP];
    VDOPVDByIterr        = [VDOPVDByIterr,       VDOP];
    % x0List, y0List, z0List
    for itera = 1:length(x0List)
        errorXIteraMatrix(iterr,itera) = x0List(itera)-userECEFPosition(1);
        errorYIteraMatrix(iterr,itera) = y0List(itera)-userECEFPosition(2);
...
%%%% 有VOR/DME
disp('----------有VOR/DME----------');
errorVDByIterrSort       = sort(errorVDByIterr);
errorVDHoriByIterrSort   = sort(errorVDHoriByIterr);
errorVDVetiByIterrJSort  = sort(errorVDVetiByIterrJ);
errorVDEastByIterrJSort  = sort(errorVDEastByIterrJ);
errorVDNorthByIterrJSort = sort(errorVDNorthByIterrJ);

%%%%%%%% 95%误差+DOP %%%%%%%%
disp(['error(95%)       = ', num2str(errorVDByIterrSort(paraAll.iterrT*0.95))]);
disp(['errorHori(95%)   = ', num2str(errorVDHoriByIterrSort(paraAll.iterrT*0.95))]);    
disp(['errorVetiJ(95%)  = ', num2str(errorVDVetiByIterrJSort(paraAll.iterrT*0.95))]); 
disp(['errorEastJ(95%)  = ', num2str(errorVDEastByIterrJSort(paraAll.iterrT*0.95))]); 
disp(['errorNorthJ(95%) = ', num2str(errorVDNorthByIterrJSort(paraAll.iterrT*0.95))]); 
disp(['error(65%)       = ', num2str(errorVDByIterrSort(paraAll.iterrT*0.65))]);
disp(['errorHori(65%)   = ', num2str(errorVDHoriByIterrSort(paraAll.iterrT*0.65))]);    
disp(['errorVetiJ(65%)  = ', num2str(errorVDVetiByIterrJSort(paraAll.iterrT*0.65))]); 
disp(['errorEastJ(65%)  = ', num2str(errorVDEastByIterrJSort(paraAll.iterrT*0.65))]); 
disp(['errorNorthJ(65%) = ', num2str(errorVDNorthByIterrJSort(paraAll.iterrT*0.65))]); 

disp(['PDOP             = ', num2str(mean(PDOPVDByIterr))]);
disp(['HDOP             = ', num2str(mean(HDOPVDByIterr))]);
disp(['VDOP             = ', num2str(mean(VDOPVDByIterr))]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
disp(['error_std        =      ', num2str(sqrt(var(errorVDXByIterr)+var(errorVDYByIterr)+var(errorVDZByIterr)))]);
disp(['posCRLB          =      ', num2str(posCRLB)]);

disp(['errorX_std       =      ', num2str(sqrt(var(errorVDXByIterr)))]);
disp(['errorX_mean      =      ', num2str(mean(errorVDXByIterr))]);
disp(['errorY_std       =      ', num2str(sqrt(var(errorVDYByIterr)))]);
disp(['errorY_mean      =      ', num2str(mean(errorVDYByIterr))]);
disp(['errorZ_std       =      ', num2str(sqrt(var(errorVDZByIterr)))]);
disp(['errorZ_mean      =      ', num2str(mean(errorVDZByIterr))]);
disp(['errorX_CRLB      =      ', num2str(sqrt(CRLBMatrix(1,1)))]);
disp(['errorY_CRLB      =      ', num2str(sqrt(CRLBMatrix(2,2)))]);
disp(['errorZ_CRLB      =      ', num2str(sqrt(CRLBMatrix(3,3)))]);

disp(['errorEast_std    =      ', num2str(sqrt(var(errorVDEastByIterr  )))]);
disp(['errorEast_mean   =      ', num2str(mean(errorVDEastByIterr    ))]);
disp(['errorNorth_std   =      ', num2str(sqrt(var(errorVDNorthByIterr )))]);
disp(['errorNorth_mean  =      ', num2str(mean(errorVDNorthByIterr    ))]);
disp(['errorVeti_std    =      ', num2str(sqrt(var(errorVDVetiByIterr  )))]);
disp(['errorVeti_mean   =      ', num2str(mean(errorVDVetiByIterr  ))]);
disp(['errorEast_CRLB   =      ', num2str(sqrt(CRLBMatrix_ENU(1,1)))]);
disp(['errorNorthY_CRLB =      ', num2str(sqrt(CRLBMatrix_ENU(2,2)))]);
disp(['errorVeti_CRLB   =      ', num2str(sqrt(CRLBMatrix_ENU(3,3)))]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure(2)  
    ...
    hold on;
    scatter(errorVDEastByIterr, errorVDNorthByIterr, pointSize, 'filled', 'g');
    xlabel('东向误差/米', FontSize=fontSize);
    ylabel('北向误差/米', FontSize=fontSize);
    legend('无VOR/DME辅助','有VOR/DME辅助');
    ax1 = gca;
    ...
    hold on;
    scatter(errorVDNorthByIterr, errorVDVetiByIterr, pointSize, 'filled', 'g');
