...
userECEFPositionStart, B00In, B10In, clcOrder)                              % 最小二乘初始输入值
    ...
    while(1)
        ...
        if(withClcFlag)
            for ns = 1:Ns
                clc0     = B00 + B10*(ttList(ns) - t1);
                deltaClc = clcList(ns) - clc0;
                dClcdx   = 0;
                dClcdy   = 0;
                dClcdz   = 0;
                dClcdB0  = 1;
                dClcdB1  = ttList(ns) - t1;
                Z        = [Z; deltaClc];
                H        = [H; dClcdx, dClcdy, dClcdz, dClcdB0, dClcdB1];
                if(weightFlag)
                    W_v = [W_v, 1/clcError^2];
                end
            end
        end
